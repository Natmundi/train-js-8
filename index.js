...
function customFill(arr, value, start = 0, end = arr.length) {
  ...
  if (!Array.isArray(arr)) {
    return false;
  } else if (start < 0 || start > arr.length || end < 0 || end > arr.length) {
    return false;
  }
...
function customAt(arr, index) {
  ...
  let indexElement = arr.at(index);

  if (typeof indexElement === "string") {
    console.log("Елемент є рядком");
  } else if (typeof indexElement === "number") {
    console.log("Елемент є числом");
  } else if (typeof indexElement === "object") {
...
function customIncludes(arr, element) {
  ...
  if (arr.includes(element)) {
    const result = arr.filter((item) => item === element).length;
    console.log(result);
    return true;
  } else {
    return false;
